      ...
      fn = function(values) {
        ...
        for (j=0,len=tokens.length; j<len; ++j) {
          ...
          if (Buffer.isBuffer(val)) {
            ...
            ret += "'";
          } else if (Array.isArray(val)) { /* start -- permit "field IN (?)" to expand an array of values -- japhy- */
            var vals = [];
            val.forEach(function (v) {
              if (Buffer.isBuffer(v))
                vals.push("'" + addon.escape(v.toString('utf8')) + "'");
              else if (val !== null)
                vals.push("'" + addon.escape(v + '') + "'");
              else
                vals.push('NULL');
            });
            ret += vals.join(','); /* end -- japhy- */
          } else if (val !== null) {
            ret += "'";
